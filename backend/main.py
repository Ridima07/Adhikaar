...
# ... keep the rest of your main.py file unchanged belowfrom fastapi import FastAPI, HTTPException, Query
from pydantic import BaseModel
from typing import Optional
...
@app.get("/")
def home(lang: str = Query("en", description="Supported: en, hi, bn")):
    response = {
...
def get_recommendations(
    ...
    recommendations = recommend_schemes(user_profile.model_dump())
    response = {
    ...
    for scheme in schemes:
        if str(scheme.get("scheme_id", "")).upper() == scheme_id.upper():
            ...
            result = {
                ...
                "additional_conditions": eligibility.get("other_conditions", []),
            ...
            return translate_data(result, target_lang=lang)
...
def scheme_rejection_recovery(
    ...
    try:
        result = recover_from_rejection(scheme_id, request.rejection_message)
        return translate_data(result, target_lang=lang)
